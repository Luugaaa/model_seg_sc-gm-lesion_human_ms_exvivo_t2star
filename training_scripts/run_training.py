...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,
                                    dataset_json=dataset_json, device=device, use_ram_caching=cache)
...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p, disable_checkpointing, c, val,
            pretrained_weights, npz, val_with_best, world_size, cache):
    setup_ddp(rank, world_size)
    torch.cuda.set_device(torch.device('cuda', dist.get_rank()))

    nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, tr, p, cache=cache)
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        ...
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, trainer_class_name,
                                               plans_identifier, device=device, cache=cache)
...
def run_training_entry():
    ...
    parser.add_argument('-cache', type=str, default='', required=False,
                    help="Set this to 'ram' to cache the dataset in the ram for faster training.")
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 ...
                 device=device, cache=args.cache)
